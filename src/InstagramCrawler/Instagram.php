...
use InstagramCrawler\Exception\InstagramChallengeRecaptchaException;
use InstagramCrawler\Exception\InstagramChallengeSubmitPhoneNumberException;
use InstagramCrawler\Exception\InstagramException;
use InstagramCrawler\Exception\InstagramNotFoundException;
use InstagramCrawler\Exception\InstagramAgeRestrictedException;
use InstagramCrawler\Http\Response;
use InstagramCrawler\Model\Account;
use InstagramCrawler\Model\Activity;
use InstagramCrawler\Model\Comment;
use InstagramCrawler\Model\Like;
use InstagramCrawler\Model\Location;
use InstagramCrawler\Model\Media;
use InstagramCrawler\Model\Story;
use InstagramCrawler\Model\Tag;
use InstagramCrawler\Model\Thread;
use InstagramCrawler\Model\UserStories;
use InstagramCrawler\Model\Highlight;
use InstagramCrawler\TwoStepVerification\ConsoleVerification;
use InstagramCrawler\TwoStepVerification\TwoStepVerificationInterface;
use InvalidArgumentException;
use InstagramCrawler\Http\Request;
use JsonException;
...
class Instagram
{
    ...
    public const MAX_COMMENTS_PER_REQUEST = 300;
    public const MAX_LIKES_PER_REQUEST = 300;
    public const PAGING_TIME_LIMIT_SEC = 1800; // 30 mins time limit on operations that require multiple requests
    public const PAGING_DELAY_MINIMUM_MICROSEC = 1000000; // 1 sec min delay to simulate browser
    public const PAGING_DELAY_MAXIMUM_MICROSEC = 3000000; // 3 sec max delay to simulate browser
     ...
     * @throws InstagramNotFoundException
     */
    public function searchAccountsByUsername(string $username, int $count = 10): array
    {
        $response = Request::get(
            Endpoints::getGeneralSearchJsonLink($username, $count),
     ...
     * @return Places[]
     * @throws InstagramException
     * @throws InstagramNotFoundException|ClientExceptionInterface
     */
    public function searchPlacesByLocationName($locationName, $count = 10): array
    {
        $response = Request::get(
            Endpoints::getGeneralSearchJsonLink($locationName, $count),
            $this->generateHeaders($this->userSession)
        );
    ...
    {
        return json_decode($rawBody, true, 512, JSON_THROW_ON_ERROR | JSON_BIGINT_AS_STRING);
    }
     ...
     * @throws InstagramNotFoundException
     */
    public function getTaggedMedias(string $username, int $count = 20, string $maxId = ''): array
    {
        $account = $this->getAccountInfo($username);
     ...
     * @throws InstagramNotFoundException
     */
    public function getTaggedMediasByUserId($id, $count = 12, $maxId = ''): array
    {
        $index = 0;
        $medias = [];
        $isMoreAvailable = true;
        while ($index < $count && $isMoreAvailable) {
            ...
            $response = Request::get(
                Endpoints::getAccountTaggedMediasJsonLink($variables),
                $this->generateHeaders($this->userSession, $this->generateGisToken($variables))
            );
...
//        return md5(implode(':', [$this->getRhxGis(), $variables]));
    ...
    public function getMediaCommentsByCode($code, $count = 10, $maxId = null, $paginateInd = false)
    {
        ...
        while ($hasPrevious && $index < $count) {
            ...
            if (
                ...
                || !array_key_exists(
                    'end_cursor',
                    $jsonResponse['data']['shortcode_media']['edge_media_to_comment']['page_info']
                )
            ) {
                throw new InstagramException(
                    'Response code is ' . $response->code . ': ' . static::httpCodeToString($response->code) . '.' .
    ...
    public function getPaginateAllFollowing($accountId, $pageSize = 20, $nextPage = '')
    {
        ...
        if (count($edgesArray) === 0) {
            throw new InstagramException(
                'Failed to get following of account id ' . $accountId . '. The account is private.',
                static::HTTP_FORBIDDEN
            );
        }
    ...
    public function searchFollowers($accountId, $query = '')
    {
        ...
        if ($jsonResponse['status'] !== 'ok') {
            throw new InstagramException(
                'Response status is ' . $jsonResponse['status'] . ' Something went wrong. Please report issue.',
                $response->code,
     ...
     * @throws InstagramException|ClientExceptionInterface
     */
    public function login(bool $force = false, $twoStepVerificator = null)
    {
     ...
     * @throws InstagramAuthException
     */
    public function loginWithSessionId($sessionId): array
    {
     ...
     * @return bool
     */
    public function isLoggedIn($session = null): bool
    {
    ...
    private function verifyTwoStep($response, $cookies, TwoStepVerificationInterface $twoStepVerificator = null)
    {
        ...
        if (preg_match('/"challengeType":"RecaptchaChallengeForm"/', $response->raw_body, $matches)) {
            ...
            throw new InstagramChallengeSubmitPhoneNumberException(
                'Instagram asked to enter a phone number.',
                $response->code
            );
        }
        ...
        if (!preg_match('/"input_name":"security_code"/', $response->raw_body, $matches)) {
            throw new InstagramAuthException(
                'Something went wrong when try two step verification. Please report issue.', $response->code
            );
        }
    ...
    {
        $response = Request::post(
            Endpoints::getRemoveFollowerUrl($accountId),
            $this->generateHeaders($this->userSession)
        );
    ...
    {
        $mediaId = $mediaId instanceof Media ? $mediaId->getId() : $mediaId;
        $repliedToCommentId = $repliedToCommentId instanceof Comment ? $repliedToCommentId->getId(
        ) : $repliedToCommentId;

        $body = ['comment_text' => $text, 'replied_to_comment_id' => $repliedToCommentId];
        $response = Request::post(
            Endpoints::getAddCommentUrl($mediaId),
            $this->generateHeaders($this->userSession),
            $body
        );
    ...
    public function deleteComment($mediaId, $commentId)
    {
        ...
        $response = Request::post(
            Endpoints::getDeleteCommentUrl($mediaId, $commentId),
            $this->generateHeaders($this->userSession)
        );
